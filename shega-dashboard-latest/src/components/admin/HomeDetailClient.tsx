...
/* ---------------- component ---------------- */
export default function HomeDetailClient({ detail }: { detail: HomeDetail }) {
  const { t } = useI18n();
  const tr = (k: string, fb: string) => withFallback(t, k, fb);

  const [live, setLive] = useState<HomeDetail>(detail);
  ...
  useEffect(() => {
    ...
    const socket = io(`${base}`, {
      ...
      auth: { role: 'admin', homeId: detail.homeId },
    });
    ...
    socket.on('connect', () => socket.emit('join:home', detail.homeId));
    ...
    socket.on('event', (evt: UiEvent) => {
      if (!evt || evt.homeId !== detail.homeId) return;
      setToasts((prev) => {
      ...
      <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-3">
        <div>
          <div className="flex items-center gap-2 text-sm text-gray-500">
            <Link href="/admin/homes" className="inline-flex items-center gap-1 text-indigo-600 hover:underline">
              <ArrowLeft className="w-4 h-4" /> {tr('homes.back', 'Back')}
            </Link>
          </div>
